...
import { IStargateFarmingPool, IVault, StrategyProxy, UsdtStargateStrategyMainnet, VaultV1 } from '../../../src/types';
import {
...
const strategyName = 'UsdtStargate';
describe(strategyName, () => {

  let core: CoreProtocol;
  let strategyProxy: StrategyProxy;
  let strategyMainnet: UsdtStargateStrategyMainnet;
  let vaultV1: VaultV1;
  ...
  const underlyingToken = STARGATE_S_USDT;
  const poolId = 2;
  const rewardPid = 1;
  ...
  before(async () => {
    ...
    [strategyProxy, strategyMainnet] = await createStrategy<UsdtStargateStrategyMainnet>('UsdtStargateStrategyMainnet');
  ...
  describe('#deployment', () => {
    it('should work properly', async () => {
      ...
      expect(await strategyMainnet.depositToken()).to.eq(USDT.address);
  ...
  describe('deposit and compound', () => {
    it('should work', async () => {
      const usdtAmount = ethers.BigNumber.from('5000000000'); // 5,000 USDT
      await setupUSDTBalance(user, usdtAmount, STARGATE_ROUTER);
      await STARGATE_ROUTER.connect(user).addLiquidity(poolId, usdtAmount, user.address);
      ...
      expect(await rewardPoolBalanceOf(rewardPool, rewardPid, strategyProxy)).to.eq(lpBalanceAfterReservesTakenOut);
