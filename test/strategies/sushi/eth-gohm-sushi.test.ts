...
const strategyName = 'EthGOhmSushi';
describe(strategyName, () => {

  let core: CoreProtocol;
  let strategyProxy: StrategyProxy;
  let strategyMainnet: EthGOhmSushiStrategyMainnet;
  let vaultV2: VaultV2;
  ...
  const underlyingToken = ETH_G_OHM_SLP;
  const pid = 12;
  const tokenA = WETH;
  const tokenB = G_OHM;
  const tokenAAmount = ethers.BigNumber.from('1000000000000000000'); // 1 ETH
  const tokenBAmount = ethers.BigNumber.from('700000000000000000'); // 0.70 GOHM
  ...
  before(async () => {
    ...
    await (core.universalLiquidator as IUniversalLiquidatorV2)
      ...
      [WETH.address, G_OHM.address],
      SUSHI_ROUTER.address,
    );
    await (core.universalLiquidator as IUniversalLiquidatorV2)
      .connect(core.governance)
      ['configureSwap(address[],address)'](
      [G_OHM.address, WETH.address],
      SUSHI_ROUTER.address,
    );

    [strategyProxy, strategyMainnet] = await createStrategy<EthGOhmSushiStrategyMainnet>('EthGOhmSushiStrategyMainnet');
  ...
  describe('#deployment', () => {
    it('should work properly', async () => {
      ...
      expect(await strategyMainnet.rewardTokens()).to.eql([SUSHI.address, G_OHM.address]);
      expect(await strategyMainnet.strategist()).to.eq(core.strategist.address);
      expect(await strategyMainnet.underlying()).to.eq(ETH_G_OHM_SLP.address);
  ...
  describe('deposit, compound, and withdraw', () => {
    it('should work', async () => {
      ...
      await setupGOHMBalance(user, tokenBAmount, SUSHI_ROUTER);
      ...
      const expectedApr = ethers.BigNumber.from('515000000000000000'); // 51.5%
      const expectedApy = ethers.BigNumber.from('673000000000000000'); // 67.3%
