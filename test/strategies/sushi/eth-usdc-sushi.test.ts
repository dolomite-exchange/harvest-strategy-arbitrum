...
const strategyName = 'EthUsdcSushi';
describe(strategyName, () => {

  let core: CoreProtocol;
  let strategyProxy: StrategyProxy;
  let strategyMainnet: EthUsdcSushiStrategyMainnet;
  let vaultV2: VaultV2;
  ...
  const underlyingToken = ETH_USDC_SLP;
  const pid = 0;
  const tokenA = WETH;
  const tokenB = USDC;
  const tokenAAmount = ethers.BigNumber.from('1000000000000000000'); // 1 ETH
  const tokenBAmount = ethers.BigNumber.from('2000000000'); // 2000 USDC
  ...
  before(async () => {
    ...
    [strategyProxy, strategyMainnet] = await createStrategy<EthUsdcSushiStrategyMainnet>('EthUsdcSushiStrategyMainnet');
  ...
  describe('#deployment', () => {
    it('should work properly', async () => {
      ...
      expect(await strategyMainnet.underlying()).to.eq(ETH_USDC_SLP.address);
  ...
  describe('deposit, compound, and withdraw', () => {
    it('should work', async () => {
      ...
      await setupUSDCBalance(user, tokenBAmount, SUSHI_ROUTER);
      ...
      // Probably not worth deploying :-(
      const expectedApr = ethers.BigNumber.from('800000000000000'); // 0.08%
      const expectedApy = ethers.BigNumber.from('800000000000000'); // 0.08%
