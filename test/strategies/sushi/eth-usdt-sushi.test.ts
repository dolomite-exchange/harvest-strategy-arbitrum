...
const strategyName = 'EthUsdtSushi';
describe(strategyName, () => {

  let core: CoreProtocol;
  let strategyProxy: StrategyProxy;
  let strategyMainnet: EthUsdtSushiStrategyMainnet;
  let vaultV2: VaultV2;
  ...
  const underlyingToken = ETH_USDT_SLP;
  const pid = 4;
  const tokenA = WETH;
  const tokenB = USDT;
  const tokenAAmount = ethers.BigNumber.from('1000000000000000000'); // 1 ETH
  const tokenBAmount = ethers.BigNumber.from('2000000000'); // 2000 USDT
  ...
  before(async () => {
    ...
    [strategyProxy, strategyMainnet] = await createStrategy<EthUsdtSushiStrategyMainnet>('EthUsdtSushiStrategyMainnet');
  ...
  describe('#deployment', () => {
    it('should work properly', async () => {
      ...
      expect(await strategyMainnet.underlying()).to.eq(ETH_USDT_SLP.address);
  ...
  describe('deposit, compound, and withdraw', () => {
    it('should work', async () => {
      ...
      await setupUSDTBalance(user, tokenBAmount, SUSHI_ROUTER);
      ...
      const expectedApr = ethers.BigNumber.from('8800000000000000'); // 0.880%
      const expectedApy = ethers.BigNumber.from('8880000000000000'); // 0.888%
