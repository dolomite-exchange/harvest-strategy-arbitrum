...
const strategyName = 'EthSushiSushi';
describe(strategyName, () => {

  let core: CoreProtocol;
  let strategyProxy: StrategyProxy;
  let strategyMainnet: EthSushiSushiStrategyMainnet;
  let vaultV2: VaultV2;
  ...
  const underlyingToken = ETH_SUSHI_SLP;
  const pid = 2;
  const tokenA = WETH;
  const tokenB = SUSHI;
  const tokenAAmount = ethers.BigNumber.from('1000000000000000000'); // 1 ETH
  const tokenBAmount = ethers.BigNumber.from('1220000000000000000000'); // 1220 SUSHI
  ...
  before(async () => {
    ...
    console.log('\tNew vault: ', vaultV2.address);
    console.log('\tNew strategy: ', strategyProxy.address);
  ...
  describe('#deployment', () => {
    it('should work properly', async () => {
      ...
      expect(await strategyMainnet.underlying()).to.eq(ETH_SUSHI_SLP.address);
  ...
  describe('deposit, compound, and withdraw', () => {
    it('should work', async () => {
      ...
      await setupSUSHIBalance(user, tokenBAmount, SUSHI_ROUTER);
      ...
      const expectedApr = ethers.BigNumber.from('38600000000000000'); // 3.86%
      const expectedApy = ethers.BigNumber.from('39300000000000000'); // 3.93%
