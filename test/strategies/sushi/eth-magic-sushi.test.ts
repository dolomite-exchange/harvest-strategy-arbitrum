...
const strategyName = 'EthMagicSushi';
describe(strategyName, () => {

  let core: CoreProtocol;
  let strategyProxy: StrategyProxy;
  let strategyMainnet: EthMagicSushiStrategyMainnet;
  let vaultV2: VaultV2;
  ...
  const underlyingToken = ETH_MAGIC_SLP;
  const pid = 13;
  const tokenA = WETH;
  const tokenB = MAGIC;
  const tokenAAmount = ethers.BigNumber.from('1000000000000000000'); // 1 ETH
  const tokenBAmount = ethers.BigNumber.from('3300000000000000000000'); // 3300 MAGIC
  ...
  before(async () => {
    ...
    await (core.universalLiquidator as IUniversalLiquidatorV2)
      ...
      [WETH.address, MAGIC.address],
      SUSHI_ROUTER.address,
    );
    await (core.universalLiquidator as IUniversalLiquidatorV2)
      .connect(core.governance)
      ['configureSwap(address[],address)'](
      [MAGIC.address, WETH.address],
      SUSHI_ROUTER.address,
    );

    [strategyProxy, strategyMainnet] =
      await createStrategy<EthMagicSushiStrategyMainnet>('EthMagicSushiStrategyMainnet');
  ...
  describe('#deployment', () => {
    it('should work properly', async () => {
      ...
      expect(await strategyMainnet.rewardTokens()).to.eql([SUSHI.address, MAGIC.address]);
      expect(await strategyMainnet.strategist()).to.eq(core.strategist.address);
      expect(await strategyMainnet.underlying()).to.eq(ETH_MAGIC_SLP.address);
  ...
  describe('deposit, compound, and withdraw', () => {
    it('should work', async () => {
      ...
      await setupMAGICBalance(user, tokenBAmount, SUSHI_ROUTER);
      ...
      const expectedApr = ethers.BigNumber.from('540000000000000000'); // 54.0%
      const expectedApy = ethers.BigNumber.from('715000000000000000'); // 71.5%
