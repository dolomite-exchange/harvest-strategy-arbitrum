...
const strategyName = 'EthDaiSushi';
describe(strategyName, () => {

  let core: CoreProtocol;
  let strategyProxy: StrategyProxy;
  let strategyMainnet: EthDaiSushiStrategyMainnet;
  let vaultV2: VaultV2;
  ...
  const underlyingToken = ETH_DAI_SLP;
  const pid = 14;
  const tokenA = WETH;
  const tokenB = DAI;
  const tokenAAmount = ethers.BigNumber.from('1000000000000000000'); // 1 ETH
  const tokenBAmount = ethers.BigNumber.from('2000000000000000000000'); // 2000 DAI
  ...
  before(async () => {
    ...
    await (core.universalLiquidator as IUniversalLiquidatorV2)
      ...
      ['configureSwap(address[],address,bytes)'](
      [USDC.address, DAI.address],
      UNISWAP_V3_ROUTER.address,
      ethers.utils.defaultAbiCoder.encode(['uint24'], ['500']),
    );

    [strategyProxy, strategyMainnet] = await createStrategy<EthDaiSushiStrategyMainnet>('EthDaiSushiStrategyMainnet');
  ...
  describe('#deployment', () => {
    it('should work properly', async () => {
      ...
      expect(await strategyMainnet.underlying()).to.eq(ETH_DAI_SLP.address);
  ...
  describe('deposit, compound, and withdraw', () => {
    it('should work', async () => {
      ...
      await setupDAIBalance(user, tokenBAmount, SUSHI_ROUTER);
      ...
      const expectedApr = ethers.BigNumber.from('26200000000000000'); // 2.62%
      const expectedApy = ethers.BigNumber.from('26500000000000000'); // 2.65%
