...
const strategyName = 'EthMimSushi';
describe(strategyName, () => {

  let core: CoreProtocol;
  let strategyProxy: StrategyProxy;
  let strategyMainnet: EthMimSushiStrategyMainnet;
  let vaultV2: VaultV2;
  ...
  const underlyingToken = ETH_MIM_SLP;
  const pid = 9;
  const tokenA = WETH;
  const tokenB = MIM;
  const tokenAAmount = ethers.BigNumber.from('1000000000000000000'); // 1 ETH
  const tokenBAmount = ethers.BigNumber.from('1980000000000000000000'); // 1980 MIM
  ...
  before(async () => {
    ...
    [strategyProxy, strategyMainnet] = await createStrategy<EthMimSushiStrategyMainnet>('EthMimSushiStrategyMainnet');
  ...
  describe('#deployment', () => {
    it('should work properly', async () => {
      ...
      expect(await strategyMainnet.underlying()).to.eq(ETH_MIM_SLP.address);
  ...
  describe('deposit, compound, and withdraw', () => {
    it('should work', async () => {
      ...
      await setupMIMBalance(user, tokenBAmount, SUSHI_ROUTER);
      ...
      const expectedApr = ethers.BigNumber.from('36000000000000000'); // 3.6%
      const expectedApy = ethers.BigNumber.from('36600000000000000'); // 3.66%
