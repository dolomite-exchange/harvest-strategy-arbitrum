...
contract MasterChefHodlStrategy is IStrategy, BaseUpgradeableStrategy {
    ...
    /**
     * Withdraws all of the asset to the vault
     */
    ...
    /**
     * Get the reward, sell it in exchange for underlying, invest what you got. It's not much, but it's honest work.
     *
     * Note that although `onlyNotPausedInvesting` is not added here, calling `investAllUnderlying()` effectively blocks
     * the usage of `doHardWork` when the investing is paused by governance.
     */
